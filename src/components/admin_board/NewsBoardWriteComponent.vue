<template>
  <main class="mt-3">
    <div class="container">
      <h2 class="text-center">뉴스내용넣기</h2>  

      <div class="mb-3 row">
        <label class="col-md-3 col-form-label">제목</label>
        <div class="col-md-9">
          <input type="text" class="form-control" v-model="news_board.title">
        </div>
      </div>
      ...
      <div class="mb-3 row">
        ...
        <div class="col-md-9">
          <input type="text" class="form-control" v-model="news_board.writer">
        </div>
      </div>
...
async function Insert() {
  ...
  try { 
    ...
    if (uploadedUrls.length) {
      news_board.value.image_urls = uploadedUrls // ✅ 추가
      news_board.value.content +=
    ...
    const response = await axios.post(apiEndpoint_news_board, payload)
